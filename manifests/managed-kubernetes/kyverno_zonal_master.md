...
--folder-id $FOLDER \
--name cam-kyverno \
--description "Kyverno test cluster" \
--labels cam=kyverno \
--network-id $NETWORK \
--zone $ZONE \
--subnet-id $SUBNET \
...
--version $VERSION \
--cluster-ipv4-range 10.3.0.0/16 \
--service-ipv4-range 10.4.0.0/16 \
--auto-upgrade=true \
...
--folder-id $FOLDER \
--name cam-fixed-kyverno \
--cluster-name cam-kyverno \
--description "fixed group for kyverno test cluster" \
--metadata serial-port-enable=1 \
--metadata-from-file=ssh-keys=/Users/cameda/ssh-pairs.txt \
--labels cam=kyverno \
--node-labels cam=kyverno \
--location zone=$ZONE \
...
yc k8s node-group create \
--folder-id $FOLDER \
--name cam-autoscale-kyverno \
--cluster-name cam-kyverno \
--description "autoscale group for kyverno test cluster" \
--metadata serial-port-enable=1 \
--metadata-from-file=ssh-keys=/Users/cameda/ssh-pairs.txt \
--labels cam=kyverno \
--node-labels cam=kyverno \
